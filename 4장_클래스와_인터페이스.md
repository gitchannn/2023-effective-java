...
- **민트**: 저는 상수를 도메인적 의미에 맞는 하나의 클래스에서 `private static`으로 관리하고, 다른 클래스에서 필요로 한다면 상수를 관리하는 클래스에서 값을 반환하는 메서드를 가지는 게 좋지 않나 생각합니다. 상수 역시 도메인이 변경됨에 따라 변경될 여지가 있는 **관리대상**이라고 생각하기 때문에 하나의 클래스에서 관리하는 것이 좋다고 생각합니다.
