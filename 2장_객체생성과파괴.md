...
- **테오**: 생성자에서 복잡한 작업을 하는 경우(리스트에서 어떤 객체로 매핑해서 무슨 작업을 한다 등) 생성자에 부담이 되지 않기 위해 정적 팩토리 메소드를 사용했었습니다. 
...
**이리내**: 책에는 생성자를 호출해 빌더 객체를 얻는 수도 있고 정적 팩터리를 호출해 빌더 객체를 얻는 수도 있다 하는데, 책에는 전자(생성자)의 예시코드만 있습니다. 한번 후자(정적 팩터리)의 예시 코드를 짜볼까요?
...
아래와 같은 상황에서, str3는 새로운 객체를 생성할까요? 아니면 기존의 객체를 재사용할까요 ?
기존의 객체를 재사용 한다면, str1과 str2 중 어떤 객체를 재사용 할까요 ?
```java
